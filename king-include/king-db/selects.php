 ...
 */
{
	...
	$selectspec = array(
		'columns'   => array(
			...
			'^posts.flagcount', '^posts.title', '^posts.postformat', '^posts.nsfw', '^posts.tags', 'created'     => 'UNIX_TIMESTAMP(^posts.created)', '^posts.name',
...
function king_insert_uploads($path, $format, $width = null, $height = null)
{
	qa_db_query_sub('INSERT INTO ^uploads (content, format, width, height) VALUES ($, $, #, #)',
...
{
	$query = qa_db_query_sub('SELECT * FROM ^poll WHERE postid=$ ', $pid);
	$cont  = qa_db_read_one_assoc($query, true);
 ...
 */
{
	...
	if (QA_FINAL_EXTERNAL_USERS) {
		...
		return array(
			'columns'   => array('^users.userid', 'handle', 'points', 'level', 'flags', '^users.email', 'coverblobid' => 'BINARY coverblobid', 'avatarblobid' => 'BINARY avatarblobid', 'avatarwidth', 'avatarheight'),
...
{
	return array(
		'columns'   => array('^users.userid', 'handle', 'level', 'coverblobid', 'avatarblobid'),
 ...
 */
{
	...
	return array(
		'columns'   => array('^users.userid', 'handle', 'flags', 'level', 'coverblobid', 'avatarblobid'),
		'source'    => $source,
		'arguments' => $arguments,
	);
}
