...
*/
function get_user_html($user, $coversize='1200', $class=null) {
	$html = '<div class="user-box '.$class.'">';
	$html .= '<div class="user-box-pt">';
	$html .= '<div style="background-image: url('.get_avatar($user['coverblobid'], $coversize).');" class="user-box-cover"></div>';
	$html .= '<a href="' . qa_path_html('user/'.$user['handle']) . '" class="user-box-alink">';
	if ($user['avatarblobid']) {
		$html .= '<img src="'.get_avatar($user['avatarblobid']).'" class="user-box-avatar" />';
	} else {
		$html .= '<div class="user-box-avatar" ></div>';
	}
	$html .= '</a>';
	if (isset($user['handle'])) {
		$html .= '<div class="user-box-name"><a href="' . qa_path_html('user/'.$user['handle']) . '"><h3>'.$user['handle'].'</h3></a>';
		$html .= '<span class="user-box-lvl">' . qa_user_level_string($user['level']) . '</span>';
		$html .= '</div>';
	} else {
		$html .= '<div class="user-box-name"><h3>' . qa_lang_html('main/anonymous') . '</h3>';
		$html .= '<span class="user-box-lvl">' . qa_user_level_string($user['level']) . '</span>';
		$html .= '</div>';
	}
...
function favorite_button($userid) {
	...
	if (isset($loginuserid) && $loginuserid != $userid && !QA_FINAL_EXTERNAL_USERS) {
		...
		$html = '<form method="post" action="">
				<button name="'.qa_html('favorite_'.QA_ENTITY_USER.'_'.$userid.'_'.(int)!$favorite).'" onclick="return qa_favorite_click2(this);" type="submit" class="king-follow '.($favorite ? 'active' : '').'"><i class="fas fa-plus"></i>'.($favorite ? 'unfollow' : 'follow').'</button>
...
function get_avatar($userblobid, $size='200') {


	if ($userblobid) {
		$html = qa_path_html('image', array('qa_blobid' => $userblobid, 'qa_size' => $size), qa_opt('site_url'), QA_URL_FORMAT_PARAMS);
